...
<div class="kaykay-virtual-wire kaykay-virtual-wire-input" style:--kaykay-virtual-wire-accent={accentColor}>
	<div class="virtual-wire-header">
		<span class="virtual-wire-kind">Portal In</span>
	...
	<div class="virtual-wire-channels" aria-label="Virtual wire input channels">
		{#each channels as channel (channel.id)}
			<div class="virtual-wire-channel">
				<Handle
					id={getVirtualWireInputHandleId(channel.id)}
					type="input"
					port={channel.port ?? '*'}
					accept={['*']}
					position="left"
				...
				<input
					...
					oninput={(e) => updateChannelLabel(channel.id, e.currentTarget.value)}
					onkeydown={(e) => e.stopPropagation()}
					aria-label="Virtual wire channel name"
				/>
				<small>{channel.port ?? 'any'}</small>
				<button
					type="button"
					class="virtual-wire-delete"
					data-kaykay-no-drag
					onclick={(e) => removeChannel(e, channel.id)}
					disabled={flow.locked || channels.length <= 1}
...
<style>
	...
	:global(.virtual-wire-socket.kaykay-handle-left) {
		left: -10px !important;
	}
